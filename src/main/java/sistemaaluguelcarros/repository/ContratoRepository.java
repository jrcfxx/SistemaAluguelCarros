...
public interface ContratoRepository extends CrudRepository<Contrato, Long> {
    ...
    @Query(
            "SELECT c FROM Contrato c JOIN FETCH c.pedido p JOIN FETCH p.cliente WHERE c.id = :id"
    )
    Optional<Contrato> buscarPorIdComPedidoECliente(Long id);

    @Query(
            "SELECT c FROM Contrato c JOIN FETCH c.pedido p JOIN FETCH p.cliente WHERE p.id = :pedidoId AND p.cliente.id = :clienteId"
    )
